export default class Icons {
  static birthdayCake({ className }) {
    return (
      <svg
        fill="oklch(0.68 0.2 34.29)"
        className={className}
        viewBox="0 0 32 32"
        data-name="Layer 2"
        id="Layer_2"
        xmlns="http://www.w3.org/2000/svg"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          <title></title>
          <path d="M20.65,26.08l-.06-.18-.08-.16-.12-.14a.92.92,0,0,0-1.32,0l-.12.14-.08.16a1.27,1.27,0,0,0,0,.18.6.6,0,0,0,0,.19.92.92,0,0,0,.07.35,1,1,0,0,0,.21.3.82.82,0,0,0,.3.21,1,1,0,0,0,.36.07l.18,0,.18-.06.16-.08a.59.59,0,0,0,.14-.12.82.82,0,0,0,.21-.3.92.92,0,0,0,.07-.35A1.23,1.23,0,0,0,20.65,26.08Z"></path>
          <path d="M13.18,26.08l-.06-.18L13,25.74l-.12-.14a.92.92,0,0,0-1.32,0l-.12.14-.08.16a1.27,1.27,0,0,0,0,.18.6.6,0,0,0,0,.19.92.92,0,0,0,.07.35,1,1,0,0,0,.21.3.82.82,0,0,0,.3.21,1,1,0,0,0,.36.07l.18,0,.18-.06.16-.08a.59.59,0,0,0,.14-.12.82.82,0,0,0,.21-.3.92.92,0,0,0,.07-.35A1.23,1.23,0,0,0,13.18,26.08Z"></path>
          <path d="M9.45,22.34l-.06-.18L9.31,22l-.12-.14a.92.92,0,0,0-1.32,0L7.75,22l-.08.16a1.27,1.27,0,0,0,0,.18.6.6,0,0,0,0,.19.92.92,0,0,0,.07.35,1,1,0,0,0,.21.3.82.82,0,0,0,.3.21,1,1,0,0,0,.36.07l.18,0,.18-.06L9,23.3a.59.59,0,0,0,.14-.12.82.82,0,0,0,.21-.3.92.92,0,0,0,.07-.35A1.23,1.23,0,0,0,9.45,22.34Z"></path>
          <path d="M16.92,22.34l-.06-.18L16.78,22l-.12-.14a.92.92,0,0,0-1.32,0l-.12.14-.08.16a1.27,1.27,0,0,0,0,.18.6.6,0,0,0,0,.19.92.92,0,0,0,.07.35,1,1,0,0,0,.21.3.82.82,0,0,0,.3.21,1,1,0,0,0,.36.07l.18,0,.18-.06.16-.08a.59.59,0,0,0,.14-.12.82.82,0,0,0,.21-.3.92.92,0,0,0,.07-.35A1.23,1.23,0,0,0,16.92,22.34Z"></path>
          <path d="M19.72,17.25l-.06-.18-.08-.16-.12-.14a.92.92,0,0,0-1.32,0l-.12.14-.08.16a1.27,1.27,0,0,0,0,.18.6.6,0,0,0,0,.19.92.92,0,0,0,.07.35,1,1,0,0,0,.21.3.82.82,0,0,0,.3.21,1,1,0,0,0,.36.07l.18,0,.18-.06.16-.08a.59.59,0,0,0,.14-.12.82.82,0,0,0,.21-.3.92.92,0,0,0,.07-.35A1.23,1.23,0,0,0,19.72,17.25Z"></path>
          <path d="M14.12,17.25l-.06-.18L14,16.92l-.12-.14a.92.92,0,0,0-1.32,0l-.12.14-.08.16a1.27,1.27,0,0,0,0,.18.6.6,0,0,0,0,.19.92.92,0,0,0,.07.35,1,1,0,0,0,.21.3.82.82,0,0,0,.3.21,1,1,0,0,0,.36.07l.18,0,.18-.06.16-.08a.59.59,0,0,0,.14-.12.82.82,0,0,0,.21-.3.92.92,0,0,0,.07-.35A1.23,1.23,0,0,0,14.12,17.25Z"></path>
          <path d="M24.39,22.34l-.06-.18L24.25,22l-.12-.14a.92.92,0,0,0-1.32,0l-.12.14-.08.16a1.27,1.27,0,0,0,0,.18.6.6,0,0,0,0,.19.92.92,0,0,0,.07.35,1,1,0,0,0,.21.3.82.82,0,0,0,.3.21,1,1,0,0,0,.36.07l.18,0,.18-.06L24,23.3a.59.59,0,0,0,.14-.12.82.82,0,0,0,.21-.3.92.92,0,0,0,.07-.35A1.23,1.23,0,0,0,24.39,22.34Z"></path>
          <path d="M7.6,17.86a.93.93,0,0,0,.93-.93V16a2.34,2.34,0,0,0,1.86.95,2.61,2.61,0,0,0,2.15-1.31c.35-.47.46-.56.65-.56s.3.09.65.56A2.61,2.61,0,0,0,16,16.93a2.61,2.61,0,0,0,2.15-1.31c.35-.47.46-.56.65-.56s.3.09.66.56a2.61,2.61,0,0,0,2.15,1.31,2.35,2.35,0,0,0,1.87-1v.95a.93.93,0,1,0,1.87,0V13.2a2.8,2.8,0,0,0-1.86-2.62v-2l.39-.39h0a3.74,3.74,0,0,0,0-5.28l-.66-.66a.93.93,0,0,0-1.32,0l-.66.66a3.74,3.74,0,0,0,0,5.28l.39.39V10.4H16.94V8.6l.39-.39a3.74,3.74,0,0,0,0-5.28l-.66-.66a.93.93,0,0,0-1.32,0l-.66.66a3.73,3.73,0,0,0,0,5.28l.39.39V10.4H10.4V8.6l.39-.39a3.73,3.73,0,0,0,0-5.28l-.66-.66a.93.93,0,0,0-1.32,0l-.66.66a3.74,3.74,0,0,0,0,5.28l.39.39v2A2.8,2.8,0,0,0,6.67,13.2v3.73A.93.93,0,0,0,7.6,17.86ZM16,4.26A1.87,1.87,0,0,1,16,6.9a1.87,1.87,0,0,1,0-2.64Zm6.54,0h0a1.87,1.87,0,0,1,0,2.64h0A1.87,1.87,0,0,1,22.54,4.26Zm-13.07,0a1.87,1.87,0,0,1,0,2.64A1.87,1.87,0,0,1,9.47,4.26Zm0,8H22.53a.93.93,0,0,1,.93.93v.21a3.29,3.29,0,0,0-1.21,1.1c-.35.47-.46.56-.66.56s-.3-.09-.66-.56a2.61,2.61,0,0,0-2.15-1.31,2.61,2.61,0,0,0-2.15,1.31c-.35.47-.46.56-.65.56s-.3-.09-.65-.56a2.61,2.61,0,0,0-2.15-1.31A2.61,2.61,0,0,0,11,14.5c-.35.47-.46.56-.65.56s-.3-.09-.65-.56a3.29,3.29,0,0,0-1.21-1.1V13.2A.93.93,0,0,1,9.47,12.26Z"></path>
          <path d="M29.07,28.13h-.93V21.6a2.8,2.8,0,0,0-2.8-2.8H6.67a2.8,2.8,0,0,0-2.8,2.8v6.53H2.93a.93.93,0,1,0,0,1.87H29.07a.93.93,0,1,0,0-1.87ZM6.67,20.67H25.33a.93.93,0,0,1,.93.93v1.07a3.78,3.78,0,0,0-1.6,1.07c-.46.46-.68.66-1.21.66s-.75-.2-1.21-.66a3.25,3.25,0,0,0-5.05,0c-.46.46-.68.66-1.21.66s-.75-.2-1.21-.66a3.25,3.25,0,0,0-5.05,0c-.46.46-.68.66-1.2.66s-.75-.2-1.21-.66a3.78,3.78,0,0,0-1.59-1.07V21.6A.93.93,0,0,1,6.67,20.67Zm-.93,7.46V24.8l.27.26a3.25,3.25,0,0,0,5.05,0c.46-.46.68-.66,1.2-.66s.75.2,1.21.66a3.25,3.25,0,0,0,5.05,0c.46-.46.68-.66,1.21-.66s.75.2,1.21.66a3.25,3.25,0,0,0,5.06,0l.28-.27v3.34Z"></path>
        </g>
      </svg>
    );
  }

  static person({ className }) {
    return (
      <svg
        viewBox="0 0 24 24"
        xmlns="http://www.w3.org/2000/svg"
        fill="oklch(0.68 0.2 34.29)"
        className={className}
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          <path
            d="M19.7454 4C20.988 4 21.9954 5.00736 21.9954 6.25V12.8051C21.551 12.3795 21.0463 12.0164 20.4954 11.7298V6.25C20.4954 5.83579 20.1596 5.5 19.7454 5.5H4.25C3.83579 5.5 3.5 5.83579 3.5 6.25V17.7546C3.5 18.1688 3.83579 18.5046 4.25 18.5046L6.999 18.504L7 15.75C7 14.8318 7.70711 14.0788 8.60647 14.0058L8.75 14H12.0218C11.7253 14.4632 11.4858 14.9665 11.3135 15.5H8.75C8.63165 15.5 8.53251 15.5822 8.5066 15.6927L8.5 15.75L8.499 18.504H11.0771C11.1582 19.0275 11.3019 19.5303 11.5001 20.0046H4.25C3.00736 20.0046 2 18.9972 2 17.7546V6.25C2 5.00736 3.00736 4 4.25 4H19.7454Z"
            fill="oklch(0.68 0.2 34.29)"
          ></path>
          <path
            d="M12 7.00046C13.6569 7.00046 15 8.34361 15 10.0005C15 11.6573 13.6569 13.0005 12 13.0005C10.3431 13.0005 9 11.6573 9 10.0005C9 8.34361 10.3431 7.00046 12 7.00046ZM12 8.50046C11.1716 8.50046 10.5 9.17203 10.5 10.0005C10.5 10.8289 11.1716 11.5005 12 11.5005C12.8284 11.5005 13.5 10.8289 13.5 10.0005C13.5 9.17203 12.8284 8.50046 12 8.50046Z"
            fill="oklch(0.68 0.2 34.29)"
          ></path>
          <path
            d="M23 17.5C23 20.5376 20.5376 23 17.5 23C14.4624 23 12 20.5376 12 17.5C12 14.4624 14.4624 12 17.5 12C20.5376 12 23 14.4624 23 17.5ZM18.0554 14.4206C17.8806 13.8598 17.1194 13.8598 16.9446 14.4206L16.3876 16.2077H14.5851C14.0194 16.2077 13.7842 16.9623 14.2418 17.3089L15.7001 18.4134L15.1431 20.2004C14.9683 20.7612 15.584 21.2276 16.0417 20.881L17.5 19.7766L18.9583 20.881C19.416 21.2276 20.0317 20.7612 19.8569 20.2004L19.2999 18.4134L20.7582 17.3089C21.2158 16.9623 20.9806 16.2077 20.4149 16.2077H18.6124L18.0554 14.4206Z"
            fill="oklch(0.68 0.2 34.29)"
          ></path>
        </g>
      </svg>
    );
  }

  static fun({ className }) {
    return (
      <svg
        fill="oklch(0.68 0.2 34.29)"
        className={className}
        width="169px"
        height="169px"
        viewBox="0 0 512 512"
        xmlns="http://www.w3.org/2000/svg"
        strokeWidth="0.00512"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          <path
            fill="oklch(0.68 0.2 34.29)"
            d="M122.7 23.32l1.7 21.87-16.7 14.25 21.4 5.17 8.4 20.25L149 66.18l21.8-1.75-14.2-16.71 5.1-21.32-20.3 8.35-18.7-11.43zM464 32a16 16 0 0 0-16 16 16 16 0 0 0 16 16 16 16 0 0 0 16-16 16 16 0 0 0-16-16zM239.8 42.5a16 16 0 0 0-16 16 16 16 0 0 0 16 16 16 16 0 0 0 16-16 16 16 0 0 0-16-16zm183.9 6.84c-9.2 1.74-17.7 7.18-25.9 14.28-7.6 6.53-14.7 14.66-20.7 23.45-18.8 3.01-37.6 10.67-50.2 21.13-16.1 13.2-30.4 35.8-38.2 59.1-7.4 3.1-14.4 6.8-20.1 10.8-15.5 10.9-23.5 31.8-29.4 50-5.9 18.3-8.8 34.3-8.8 34.3l17.8 3.2s2.7-15 8.1-31.9c5.5-16.9 14.8-35.3 22.7-40.8 1.3-.9 3-1.9 4.5-2.8-.6 5.7-.6 11.4.3 16.8 1.8 11.4 8 22.3 19 28.2 7.8 4.2 16.6 3.2 24 .2 7.4-3.1 14-8.2 19.7-14.2 5.7-6 10.4-13.1 13.2-20.6 2.8-7.5 3.8-16 .2-23.9h-.1c-3.9-8.4-11.4-13.8-19.4-16.1-8-2.3-16.6-2.2-25.2-.9-1.5.2-2.9.7-4.4 1 7.4-15.8 18-30.7 27.5-38.6 6.2-5.1 16.6-10 27.7-13.6-1.4 3.8-2.5 7.6-3 11.5-1.6 10.5.7 21.9 9.1 29.7 6.1 5.6 14.3 6.5 21.5 5.3 7.1-1.2 14-4.4 20.2-8.5 6.2-4.2 11.7-9.4 15.6-15.5 3.9-6.1 6.5-13.9 4-21.7v-.1c-3.3-10.07-11.5-16.99-20.6-20.27-3.9-1.4-8-2.19-12.2-2.66 2.9-3.26 5.9-6.31 8.9-8.92 6.8-5.84 13.7-9.5 17.6-10.23l-3.4-17.68zM174.8 84.39l-15.2 9.56 34.5 55.25-56.4 2.9 26.5 57.8 16.4-7.6-15.5-33.6 60.6-3.1-50.9-81.21zm216.4 19.31c6.1-.1 11.5.6 15.5 2.1 5.4 1.9 8.1 4.3 9.5 8.8.4 1.1.2 3.3-1.9 6.6-2.2 3.4-6.1 7.2-10.5 10.2-4.5 3-9.5 5.1-13.2 5.7-3.8.7-5.5 0-6.3-.7-3.5-3.2-4.5-7.2-3.5-13.9.8-5.4 3.3-11.9 7-18.6 1.2 0 2.3-.2 3.4-.2zM94.99 123a16 16 0 0 0-16 16 16 16 0 0 0 16 16A16 16 0 0 0 111 139a16 16 0 0 0-16.01-16zm356.11 37.2l-14.4 16.6-21.8-1.8 11.4 18.8-8.5 20.2 21.4-5 16.6 14.3 1.9-21.9 18.7-11.4-20.2-8.5-5.1-21.3zm-123.5 16.5c2.9.1 5.6.5 7.7 1.1 4.3 1.2 6.6 3 8.2 6.4.9 1.9 1 5.4-.7 10-1.7 4.7-5.2 10.1-9.4 14.6s-9.3 8.1-13.5 9.8c-4.2 1.7-6.8 1.6-8.5.7h-.1c-5.8-3.2-8.6-7.8-9.7-15.2-1-6.3-.3-14.3 1.8-22.9 4.9-1.7 9.8-3.1 14.5-3.8 3.5-.5 6.7-.7 9.7-.7zm-202.4 51.9c-7.2-.2-11.7 1.5-14.5 4.3-2.8 2.8-4.5 7.3-4.3 14.5.2 7.3 2.6 16.9 7.2 27.6 9.2 21.5 27.3 47.4 51.6 71.8 24.3 24.3 50.3 42.3 71.8 51.5 10.6 4.6 20.2 7 27.5 7.2 7.3.3 11.7-1.5 14.5-4.3 2.8-2.8 4.6-7.2 4.3-14.5-.2-7.3-2.6-16.9-7.2-27.6-9.2-21.4-27.2-47.4-51.5-71.7-24.3-24.4-50.3-42.4-71.8-51.6-10.7-4.6-20.3-7-27.6-7.2zm232 31.3l-33 54-29.1-27.9-12.4 13 45.1 43.3 33.8-55.2 38.7 32.3 89.3-38.2-7-16.6-79.3 34-46.1-38.7zM93.43 272.6l-17.64 57.9c41.41 49.1 89.71 76.7 142.11 94.7l21.6-6.6c-3.1-1.1-6.4-2.4-9.7-3.8-24.4-10.4-51.7-29.6-77.3-55.3-25.7-25.7-44.9-53-55.34-77.4-1.41-3.2-2.65-6.4-3.73-9.5zm-23.82 78.2l-14.01 46c28.89 27 59 39.2 90.6 50.2l43.4-13.2c-43.2-17.6-84-43.3-119.99-83zM368 352a16 16 0 0 0-16 16 16 16 0 0 0 16 16 16 16 0 0 0 16-16 16 16 0 0 0-16-16zM49.81 415.9l-20.29 66.6 88.28-26.9c-22.77-9.1-45.78-20.7-67.99-39.7z"
          ></path>
        </g>
      </svg>
    );
  }
  static NavStar({ className }) {
    return (
      <svg
        className={className}
        width="75px"
        height="45px"
        viewBox="-1.92 -1.92 27.84 27.84"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        stroke="#fb5c38"
        strokeWidth="0.00024000000000000003"
        transform="rotate(0)"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M11.0137 2.76683C11.379 1.89022 12.6208 1.89022 12.9861 2.76683L14.9102 7.38462C15.0654 7.75726 15.4295 8 15.8332 8H20.893C21.8234 8 22.2893 9.12483 21.6314 9.78268L17.5391 13.875C17.2823 14.1318 17.185 14.5076 17.2847 14.8568L18.9076 20.5369C19.1816 21.496 18.1122 22.2767 17.2822 21.7234L12.5546 18.5716C12.2187 18.3477 11.7811 18.3477 11.4452 18.5717L6.72544 21.7182C5.89284 22.2732 4.81988 21.49 5.09479 20.5279L6.71509 14.8568C6.81486 14.5076 6.71747 14.1318 6.46068 13.875L2.38859 9.8029C1.72328 9.13758 2.19448 8 3.13538 8H8.16658C8.57028 8 8.93438 7.75726 9.08965 7.38462L11.0137 2.76683Z"
            fill="#fb5c38"
          ></path>
        </g>
      </svg>
    );
  }

  static StarBadge({ className }) {
    return (
      <svg
        className={className}
        width="188px"
        height="188px"
        viewBox="-0.72 -0.72 25.44 25.44"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M6.39113 10.5439C6.39051 9.44976 6.7016 8.37924 7.28617 7.46394C7.86503 6.55596 8.69417 5.84514 9.6681 5.42194C10.9668 4.85935 12.4322 4.85935 13.7309 5.42194C14.7052 5.84498 15.5347 6.55581 16.1138 7.46394C17.3021 9.32579 17.309 11.731 16.1314 13.5999L18.525 17.9719L16.0037 17.4719L15.1973 19.9999L12.9753 15.9249C12.1382 16.1414 11.2618 16.1414 10.4247 15.9249L8.20267 19.9999L7.39635 17.4709L4.875 17.9719L7.26863 13.5999C6.69465 12.6895 6.38988 11.6281 6.39113 10.5439V10.5439Z"
            stroke=" oklch(0.32 0.14 287.43)"
            strokeWidth="0.8399999999999999"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>
          <path
            fillRule="evenodd"
            clipRule="evenodd"
            d="M11.4504 8.32691C11.4973 8.22925 11.5943 8.16742 11.7005 8.16742C11.8066 8.16742 11.9036 8.22925 11.9506 8.32691L12.4683 9.37291C12.5069 9.45209 12.5788 9.50881 12.6633 9.52691L13.7475 9.74991C13.8468 9.77396 13.9266 9.84932 13.9583 9.94877C13.99 10.0482 13.9688 10.1574 13.9025 10.2369L13.1284 11.1369C13.0738 11.2009 13.0483 11.2856 13.0582 11.3699L13.2015 12.5879C13.216 12.6944 13.1719 12.8004 13.0868 12.8633C13.0018 12.9262 12.8898 12.9357 12.7959 12.8879L11.8209 12.3799C11.7419 12.3384 11.6483 12.3384 11.5693 12.3799L10.5943 12.8879C10.5004 12.9357 10.3884 12.9262 10.3034 12.8633C10.2183 12.8004 10.1742 12.6944 10.1887 12.5879L10.3321 11.3729C10.3419 11.2886 10.3164 11.2039 10.2619 11.1399L9.4877 10.2399C9.41982 10.159 9.39893 10.0473 9.43286 9.94637C9.46679 9.84548 9.55041 9.77067 9.65248 9.74991L10.7367 9.52591C10.8212 9.50781 10.893 9.45109 10.9317 9.37191L11.4504 8.32691Z"
            stroke=" oklch(0.32 0.14 287.43)"
            strokeWidth="0.8399999999999999"
            strokeLinecap="round"
            strokeLinejoin="round"
          ></path>
          <path
            d="M7.90164 13.1977C7.6795 12.8481 7.21601 12.7448 6.8664 12.9669C6.51679 13.189 6.41345 13.6525 6.63559 14.0021L7.90164 13.1977ZM7.95112 14.4679L8.49338 13.9498L8.49105 13.9474L7.95112 14.4679ZM10.1224 15.8399L10.3567 15.1274L10.3557 15.1271L10.1224 15.8399ZM10.2422 16.6561C10.6433 16.7596 11.0523 16.5184 11.1558 16.1173C11.2593 15.7162 11.018 15.3072 10.6169 15.2037L10.2422 16.6561ZM16.7644 14.0021C16.9865 13.6525 16.8832 13.189 16.5336 12.9669C16.184 12.7448 15.7205 12.8481 15.4983 13.1977L16.7644 14.0021ZM15.4489 14.4679L14.9089 13.9474L14.9066 13.9498L15.4489 14.4679ZM13.2775 15.8399L13.0443 15.1271L13.0433 15.1274L13.2775 15.8399ZM12.783 15.2037C12.382 15.3072 12.1407 15.7162 12.2442 16.1173C12.3477 16.5184 12.7567 16.7596 13.1578 16.6561L12.783 15.2037ZM6.63559 14.0021C6.86081 14.3566 7.12062 14.6871 7.41118 14.9885L8.49105 13.9474C8.27068 13.7188 8.07317 13.4676 7.90164 13.1977L6.63559 14.0021ZM7.40886 14.986C8.0972 15.7064 8.94842 16.2449 9.88921 16.5527L10.3557 15.1271C9.65246 14.897 9.01285 14.4935 8.49337 13.9498L7.40886 14.986ZM9.88822 16.5524C10.0051 16.5908 10.1231 16.6254 10.2422 16.6561L10.6169 15.2037C10.5294 15.1811 10.4426 15.1557 10.3567 15.1274L9.88822 16.5524ZM15.4983 13.1977C15.3268 13.4676 15.1293 13.7188 14.9089 13.9474L15.9888 14.9885C16.2794 14.6871 16.5392 14.3566 16.7644 14.0021L15.4983 13.1977ZM14.9066 13.9498C14.3871 14.4935 13.7475 14.897 13.0443 15.1271L13.5108 16.5527C14.4516 16.2449 15.3028 15.7064 15.9911 14.986L14.9066 13.9498ZM13.0433 15.1274C12.9574 15.1557 12.8706 15.1811 12.783 15.2037L13.1578 16.6561C13.2769 16.6254 13.3949 16.5908 13.5118 16.5524L13.0433 15.1274Z"
            fill=" oklch(0.32 0.14 287.43)"
          ></path>
        </g>
      </svg>
    );
  }

  static Sparkles({ className }) {
    return (
      <svg
        className={className}
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        stroke="#ffffff"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          <path
            d="M12 20V17.6M12 6.4V4M20 126M6.4 124M17.6569 6.34315L15.9598 8.0402M8.0402 15.9598L6.34315 17.6569M6.34293 6.34332L8.03999 8.04038M15.9596 15.96L17.6566 17.657"
            stroke="#ffffff"
            strokeWidth="0.528"
            strokeLinecap="round"
          ></path>
        </g>
      </svg>
    );
  }

  static SlideArrow({ className }) {
    return (
      <svg
        className={className}
        fill="none"
        viewBox="-2.64 -2.64 29.28 29.28"
        xmlns="http://www.w3.org/2000/svg"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          <path d="M5.536 21.886a1.004 1.004 0 0 0 1.033-.064l13-9a1 1 0 0 0 0-1.644l-13-9A1 1 0 0 0 5 3v18a1 1 0 0 0 .536.886z"></path>
        </g>
      </svg>
    );
  }

  static Ballons({ className }) {
    return (
      <svg
        className={className}
        fill="#fb5c38"
        height="200px"
        width="200px"
        version="1.1"
        id="Capa_1"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        viewBox="0 0 470 470"
        xmlSpace="preserve"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          <g>
            <path d="M157.313,274.717c-14.169,15.228-30.787,29.689-47.223,41.13c-22.887-15.915-45.294-37.002-62.036-58.458 C26.43,229.675,15,202.492,15,178.778c0-46.609,33.306-86.041,79.193-93.762c4.085-0.687,6.839-4.556,6.152-8.64 c-0.688-4.085-4.558-6.842-8.641-6.152C66.34,74.492,43.124,87.67,26.334,107.332C9.353,127.219,0,152.592,0,178.778 c0,27.09,12.527,57.464,36.229,87.838c17.253,22.111,40.212,43.832,63.759,60.453l-10.606,17.345 c-1.416,2.315-1.471,5.214-0.144,7.581s3.829,3.832,6.542,3.832h6.813V462.5c0,4.142,3.357,7.5,7.5,7.5s7.5-3.358,7.5-7.5V355.827 h6.812c4.143,0,7.5-3.358,7.5-7.5s-3.357-7.5-7.5-7.5h-15.247l6.488-10.61c18.271-12.346,36.913-28.368,52.652-45.282 c2.821-3.032,2.65-7.778-0.382-10.6C164.88,271.513,160.135,271.685,157.313,274.717z"></path>
            <path d="M82.641,103.533C51.155,115.052,30,145.291,30,178.778c0,4.142,3.357,7.5,7.5,7.5s7.5-3.358,7.5-7.5 c0-27.216,17.198-51.794,42.795-61.158c3.89-1.423,5.89-5.73,4.467-9.62C90.839,104.109,86.534,102.109,82.641,103.533z"></path>
            <path d="M443.666,107.332c-16.789-19.662-40.005-32.84-65.37-37.108c-4.085-0.69-7.952,2.067-8.641,6.152 c-0.687,4.084,2.067,7.953,6.152,8.64C421.694,92.737,455,132.169,455,178.778c0,23.714-11.43,50.897-33.054,78.611 c-16.742,21.457-39.15,42.543-62.036,58.458c-16.438-11.442-33.057-25.904-47.223-41.13c-2.821-3.032-7.567-3.204-10.6-0.382 c-3.033,2.821-3.204,7.567-0.383,10.6c14.43,15.509,31.306,30.27,48.098,42.136l-10.605,17.344 c-1.416,2.315-1.471,5.214-0.144,7.581s3.829,3.832,6.542,3.832h6.812l0.001,106.673c0,4.142,3.358,7.5,7.5,7.5 c4.143,0,7.5-3.358,7.5-7.5l-0.001-106.673h6.813c4.143,0,7.5-3.358,7.5-7.5s-3.357-7.5-7.5-7.5h-15.247l6.492-10.618 c25.119-16.969,49.943-40.06,68.306-63.592C457.473,236.242,470,205.868,470,178.778C470,152.592,460.647,127.219,443.666,107.332z "></path>
            <path d="M320.944,230.968c27.73-35.539,42.388-71.029,42.388-102.636C363.332,57.569,305.763,0,235,0 S106.668,57.569,106.668,128.332c0,31.606,14.657,67.097,42.388,102.636c20.492,26.262,47.844,52.066,75.829,71.658l-13.139,21.488 c-1.416,2.315-1.471,5.214-0.144,7.581s3.829,3.832,6.542,3.832h9.356V462.5c0,4.142,3.357,7.5,7.5,7.5s7.5-3.358,7.5-7.5V335.527 h9.357c4.143,0,7.5-3.358,7.5-7.5s-3.357-7.5-7.5-7.5h-20.337l9.038-14.781C270.114,285.82,299.341,258.654,320.944,230.968z M160.882,221.74c-25.654-32.877-39.214-65.177-39.214-93.408C121.668,65.84,172.509,15,235,15s113.332,50.84,113.332,113.332 c0,28.231-13.56,60.531-39.214,93.408c-19.984,25.611-46.791,50.776-74.118,69.656C207.673,272.516,180.866,247.352,160.882,221.74 z"></path>
            <path d="M144.168,135.819c4.142,0,7.499-3.357,7.5-7.499C151.674,82.377,189.057,45,235,45c4.143,0,7.5-3.358,7.5-7.5 S239.143,30,235,30c-54.213,0-98.324,44.105-98.332,98.318C136.667,132.46,140.024,135.819,144.168,135.819L144.168,135.819z"></path>
            <path d="M156.799,155.297c-1.383-3.904-5.669-5.949-9.573-4.567s-5.949,5.668-4.567,9.573 c5.893,16.642,16.002,34.208,30.049,52.209c1.479,1.896,3.688,2.887,5.918,2.887c1.613,0,3.238-0.519,4.609-1.587 c3.266-2.548,3.847-7.261,1.299-10.527C171.481,186.559,162.15,170.413,156.799,155.297z"></path>
          </g>
        </g>
      </svg>
    );
  }

  static Candy({ className }) {
    return (
      <svg
        className={className}
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        transform="rotate(-45)"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          <path
            d="M3 9L3.6 8.2C3.29698 7.97274 2.89157 7.93618 2.55279 8.10557C2.214 8.27496 2 8.62123 2 9H3ZM3 15H2C2 15.3788 2.214 15.725 2.55279 15.8944C2.89157 16.0638 3.29698 16.0273 3.6 15.8L3 15ZM21 9H22C22 8.62123 21.786 8.27496 21.4472 8.10557C21.1084 7.93618 20.703 7.97274 20.4 8.2L21 9ZM21 15L20.4 15.8C20.703 16.0273 21.1084 16.0638 21.4472 15.8944C21.786 15.725 22 15.3788 22 15H21ZM2.4 9.8L6.4 12.8L7.6 11.2L3.6 8.2L2.4 9.8ZM6.4 11.2L2.4 14.2L3.6 15.8L7.6 12.8L6.4 11.2ZM4 15V9H2V15H4ZM20.4 8.2L16.4 11.2L17.6 12.8L21.6 9.8L20.4 8.2ZM16.4 12.8L20.4 15.8L21.6 14.2L17.6 11.2L16.4 12.8ZM22 15V9H20V15H22ZM16 12C16 14.2091 14.2091 16 12 16V18C15.3137 18 18 15.3137 18 12H16ZM8 12C8 9.79086 9.79086 8 12 8V6C8.68629 6 6 8.68629 6 12H8ZM12 8C14.164 8 16 9.836 16 12H18C18 8.73143 15.2686 6 12 6V8ZM12 16C9.836 16 8 14.164 8 12H6C6 15.2686 8.73143 18 12 18V16ZM15.8505 9.23479L9.2343 15.851L10.6485 17.2652L17.2648 10.649L15.8505 9.23479ZM13.3505 6.73479L6.7343 13.351L8.14851 14.7652L14.7648 8.149L13.3505 6.73479Z"
            fill="#fb5c38"
          ></path>
        </g>
      </svg>
    );
  }

  static Pizza({ className }) {
    return (
      <svg
        className={className}
        fill="#fb5c38"
        version="1.1"
        id="Layer_1"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        viewBox="0 0 260 260"
        enableBackground="new 0 0 260 260"
        xmlSpace="preserve"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          <path d="M239.467,136H258c-3.136,67.906-59.208,122-127.928,122C59.34,258,2,200.692,2,130S59.34,2,130.072,2 c21.189,0,41.173,5.147,58.777,14.252l-9.286,16.074C164.69,24.767,147.871,20.5,130.072,20.5C69.66,20.5,20.51,69.622,20.51,130 c0,60.379,49.149,109.5,109.562,109.5C188.471,239.5,236.34,193.599,239.467,136z M119.674,136h107.763 c-3.112,50.977-45.589,91.5-97.365,91.5c-45.895,0-84.482-31.841-94.825-74.579c2.193,3.609,6.152,6.029,10.686,6.029 c6.907,0,12.507-5.596,12.507-12.5s-5.6-12.5-12.507-12.5c-6.237,0-11.393,4.568-12.338,10.535 c-0.708-4.728-1.078-9.564-1.078-14.485c0-53.762,43.763-97.5,97.555-97.5c15.614,0,30.379,3.691,43.478,10.237L119.674,136z M126.57,66c0,4.142,3.36,7.5,7.504,7.5c4.145,0,7.504-3.358,7.504-7.5s-3.36-7.5-7.504-7.5C129.93,58.5,126.57,61.858,126.57,66z M83.046,88c6.907,0,12.507-5.596,12.507-12.5S89.953,63,83.046,63c-6.907,0-12.507,5.596-12.507,12.5S76.138,88,83.046,88z M83.046,129.5c4.144,0,7.504-3.358,7.504-7.5s-3.36-7.5-7.504-7.5s-7.504,3.358-7.504,7.5S78.901,129.5,83.046,129.5z M112,196.75 c0-6.904-5.6-12.5-12.507-12.5c-6.907,0-12.507,5.596-12.507,12.5s5.6,12.5,12.507,12.5C106.4,209.25,112,203.654,112,196.75z M176.568,163.875c0,6.904,5.6,12.5,12.507,12.5s12.507-5.596,12.507-12.5s-5.6-12.5-12.507-12.5S176.568,156.971,176.568,163.875z M154.586,202.938c0,4.142,3.36,7.5,7.504,7.5s7.504-3.358,7.504-7.5s-3.36-7.5-7.504-7.5S154.586,198.796,154.586,202.938z M119.066,162c0,4.142,3.36,7.5,7.504,7.5c4.144,0,7.504-3.358,7.504-7.5s-3.36-7.5-7.504-7.5 C122.426,154.5,119.066,157.858,119.066,162z M239.467,124H258c-1.973-42.727-24.902-79.986-58.756-101.741l-9.293,16.088 C218.273,56.892,237.522,88.182,239.467,124z M227.437,124h-86.966l27.703-47.957c2.078,1.537,4.64,2.457,7.424,2.457 c6.907,0,12.507-5.596,12.507-12.5c0-5.127-3.092-9.528-7.511-11.456l3.345-5.791C208.684,65.195,225.521,92.614,227.437,124z M202.613,94.438c-4.144,0-7.504,3.358-7.504,7.5s3.36,7.5,7.504,7.5s7.504-3.358,7.504-7.5S206.758,94.438,202.613,94.438z"></path>
        </g>
      </svg>
    );
  }

  static PartyInvite({ className }) {
    return (
      <svg
        className={className}
        fill="#fb5c38"
        height="200px"
        width="200px"
        version="1.1"
        id="Layer_1"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        viewBox="0 0 512 512"
        xmlSpace="preserve"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          <g>
            <g>
              <path d="M498.016,187.294L434.087,133.9v-28.215c0-21.481-17.476-38.957-38.957-38.957h-41.466l-65.159-54.421 C269.65-3.78,242.343-3.778,223.488,12.313l-65.153,54.417H116.87c-21.481,0-38.957,17.476-38.957,38.957v28.213l-63.929,53.395 C5.334,194.519,0,205.393,0,217.194v244.476c0,27.618,22.469,50.087,50.087,50.087h411.826c27.618,0,50.087-22.469,50.087-50.087 V217.194C512,205.586,506.817,194.646,498.016,187.294z M434.087,177.406l35.172,29.378l-35.172,29.377V177.406z M244.972,37.876 c0.055-0.046,0.109-0.091,0.162-0.138c6.295-5.395,15.435-5.396,21.731,0c0.053,0.047,0.108,0.092,0.162,0.138l34.547,28.853 h-91.149L244.972,37.876z M111.304,141.604c0-0.002,0-0.004,0-0.006v-35.912c0-3.069,2.497-5.565,5.565-5.565h230.717 c0.004,0,0.009,0,0.012,0c0.003,0,0.007,0,0.01,0h47.521c3.069,0,5.565,2.497,5.565,5.565v35.913c0,0.001,0,0.002,0,0.003V264.05 L267.028,375.69c-0.056,0.046-0.11,0.092-0.162,0.138c-6.295,5.397-15.435,5.396-21.731,0c-0.053-0.047-0.108-0.092-0.162-0.138 L111.304,264.049V141.604z M77.913,177.406v58.753l-35.172-29.377L77.913,177.406z M478.609,461.669 c0,9.206-7.49,16.696-16.696,16.696H50.087c-9.206,0-16.696-7.49-16.696-16.696V242.481l190.096,158.771 c18.851,16.088,46.154,16.098,65.014,0.01l139.508-116.518c0.003-0.002,0.006-0.004,0.009-0.008l50.591-42.257V461.669z"></path>
            </g>
          </g>
          <g>
            <g>
              <path d="M333.913,211.425c0-42.961-34.951-77.913-77.913-77.913c-42.961,0-77.913,34.952-77.913,77.913 c0,39.313,29.273,71.898,67.162,77.154l-7.483,14.966c-1.85,3.701,0.84,8.054,4.978,8.054h26.513c4.137,0,6.827-4.353,4.978-8.054 l-7.483-14.966C304.641,283.324,333.913,250.738,333.913,211.425z M256,255.947c-24.549,0-44.522-19.972-44.522-44.522 c0-24.549,19.972-44.522,44.522-44.522c24.549,0,44.522,19.972,44.522,44.522C300.522,235.974,280.549,255.947,256,255.947z"></path>
            </g>
          </g>
        </g>
      </svg>
    );
  }
  static TimeSand({ className }) {
    return (
      <svg
        className={className}
        fill="#fb5c38"
        viewBox="0 0 24 24"
        xmlns="http://www.w3.org/2000/svg"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          <path d="M20,3a1,1,0,0,0,0-2H4A1,1,0,0,0,4,3H5.049c.146,1.836.743,5.75,3.194,8-2.585,2.511-3.111,7.734-3.216,10H4a1,1,0,0,0,0,2H20a1,1,0,0,0,0-2H18.973c-.105-2.264-.631-7.487-3.216-10,2.451-2.252,3.048-6.166,3.194-8Zm-6.42,7.126a1,1,0,0,0,.035,1.767c2.437,1.228,3.2,6.311,3.355,9.107H7.03c.151-2.8.918-7.879,3.355-9.107a1,1,0,0,0,.035-1.767C7.881,8.717,7.227,4.844,7.058,3h9.884C16.773,4.844,16.119,8.717,13.58,10.126ZM12,13s3,2.4,3,3.6V20H9V16.6C9,15.4,12,13,12,13Z"></path>
        </g>
      </svg>
    );
  }

  static Guests({ className }) {
    return (
      <svg
        className={className}
        fill="#fb5c38"
        version="1.1"
        id="Capa_1"
        xmlns="http://www.w3.org/2000/svg"
        xmlnsXlink="http://www.w3.org/1999/xlink"
        viewBox="0 0 27.996 27.996"
        xmlSpace="preserve"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          <g>
            <g>
              <path d="M25.939,25.938l2.057-0.188l-1.078-9.75c0.115-0.852,0.268-2.143,0.326-3.478c0.098-2.276,0.439-9.503-6.114-9.503 c-6.556,0-6.377,6.034-6.377,6.034l0.478,7.83l-0.91,8.777l2.055,0.188l0.816-6.308c0.496,1.677,0.965,3.222,0.965,3.222 l-0.61,3.707h7.718l-0.869-3.836c0,0,0.269-1.775,0.592-3.644L25.939,25.938z M22.631,13.848v1.841h0.01l0.661,0.313l-2.075,3.739 l-2.033-3.854l0.52-0.131v-1.972c-2.121-0.666-3.671-2.627-3.671-4.969c0-0.177,0.033-0.344,0.051-0.516 c1.446-0.099,6.318-0.548,6.444-2.132c0.466,0.859,2.926,3.018,3.922,3.183C26.234,11.523,24.689,13.292,22.631,13.848z"></path>
              <path d="M8.887,14.506c0,0-0.697,2.141-2.616,2.254c-1.921,0.11-2.647-2.254-2.647-2.254H2.511H1.254H0.283L0,24.434h1.749 l0.092,1.901h0.268v0.133h8.578v-0.133h0.328l0.001-2.109l1.831-0.031l-0.226-9.688H8.887z"></path>
              <path d="M1.5,7.164C1.437,7.373,1.372,7.583,1.334,7.801C1.29,8.073,1.266,8.357,1.266,8.643c0,2.816,2.282,5.097,5.099,5.097 c2.812,0,5.094-2.281,5.094-5.097c0-0.563-0.112-1.095-0.279-1.602l1.034-1.428L10.36,4.96l0.362-1.896L8.456,3.711L7.415,1.528 L5.481,3.545L3.983,1.81L3.278,4.103L0.873,3.314l0.906,2.027l-1.75,0.491L1.5,7.164z"></path>
            </g>
          </g>
        </g>
      </svg>
    );
  }

  static Play({ className }) {
    return (
      <svg
        className={className}
        width="228px"
        height="228px"
        viewBox="0 0 24.00 24.00"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="0"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          {" "}
          <g clipPath="url(#clip0_429_11133)">
            {" "}
            <circle
              cx="12"
              cy="12"
              r="9"
              stroke="#ffff"
              strokeWidth="2.04"
              strokeLinecap="round"
              strokeLinejoin="round"
            ></circle>{" "}
            <path
              d="M14 12L11 13.7321L11 10.2679L14 12Z"
              stroke="#ffff"
              strokeWidth="2.04"
              strokeLinejoin="round"
            ></path>{" "}
          </g>{" "}
          <defs>
            {" "}
            <clipPath id="clip0_429_11133">
              {" "}
              <rect width="44" height="44" fill="white"></rect>{" "}
            </clipPath>{" "}
          </defs>{" "}
        </g>
      </svg>
    );
  }
  static Shield({ className }) {
    return (
      <svg
        className={className}
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M12 22C12 22 20 18 20 12V5L12 2L4 5V12C4 18 12 22 12 22Z"
          stroke="#fb5c38"
          strokeWidth="1"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    );
  }

  static Clean({ className }) {
    return (
      <svg
        className={className}
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M7 16H3V12M17 8H21V12M7 8H3V4M17 16H21V20M12 3V21"
          stroke="#fb5c38"
          strokeWidth="1"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    );
  }

  static Wifi({ className }) {
    return (
      <svg
        className={className}
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M5 12.55C6.97656 10.9037 9.46761 10.0021 12.04 10.0021C14.6124 10.0021 17.1034 10.9037 19.08 12.55M1.63 9.11C4.54 6.75 8.14 5.44 12 5.44C15.86 5.44 19.46 6.75 22.37 9.11M8.53 16.11C9.55 15.37 10.75 15 12 15C13.25 15 14.45 15.37 15.47 16.11M12 20H12.01"
          stroke="#fb5c38"
          strokeWidth="1"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    );
  }

  static NutFree({ className }) {
    return (
      <svg
        className={className}
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M19 5L5 19M9 9C9 7.34315 10.3431 6 12 6C13.6569 6 15 7.34315 15 9C15 10.6569 13.6569 12 12 12C10.3431 12 9 10.6569 9 9ZM9 15C9 13.3431 10.3431 12 12 12C13.6569 12 15 13.3431 15 15C15 16.6569 13.6569 18 12 18C10.3431 18 9 16.6569 9 15Z"
          stroke="#fb5c38"
          strokeWidth="1"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    );
  }

  static EcoFriendly({ className }) {
    return (
      <svg
        className={className}
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <path
          d="M2 22L16 8M6.5 17.5L17.5 6.5M22 12C22 17.5228 17.5228 22 12 22C6.47715 22 2 17.5228 2 12C2 6.47715 6.47715 2 12 2C17.5228 2 22 6.47715 22 12Z"
          stroke="#fb5c38"
          strokeWidth="1"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </svg>
    );
  }

  static Instagram({ className }) {
    return (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
        className={className}
      >
        <rect width="20" height="20" x="2" y="2" rx="5" ry="5"></rect>
        <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z"></path>
        <line x1="17.5" x2="17.51" y1="6.5" y2="6.5"></line>
      </svg>
    );
  }

  static Twitter({ className }) {
    return (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
        className={className}
      >
        <path d="M22 4s-.7 2.1-2 3.4c1.6 10-9.4 17.3-18 11.6 2.2.1 4.4-.6 6-2C3 15.5.5 9.6 3 5c2.2 2.6 5.6 4.1 9 4-.9-4.2 4-6.6 7-3.8 1.1 0 3-1.2 3-1.2z"></path>
      </svg>
    );
  }

  static Facebook({ className }) {
    return (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
        className={className}
      >
        <path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z"></path>
      </svg>
    );
  }

  static UpIcon({ className }) {
    return (
      <svg
        className={className}
        width="86px"
        height="86px"
        viewBox="0 0 24 24"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
      >
        <g id="SVGRepo_bgCarrier" strokeWidth="1"></g>
        <g
          id="SVGRepo_tracerCarrier"
          strokeLinecap="round"
          strokeLinejoin="round"
        ></g>
        <g id="SVGRepo_iconCarrier">
          {" "}
          <g id="Shape / Triangle">
            {" "}
            <path
              id="Vector"
              d="M4.37891 15.1999C3.46947 16.775 3.01489 17.5634 3.08281 18.2097C3.14206 18.7734 3.43792 19.2851 3.89648 19.6182C4.42204 20.0001 5.3309 20.0001 7.14853 20.0001H16.8515C18.6691 20.0001 19.5778 20.0001 20.1034 19.6182C20.5619 19.2851 20.8579 18.7734 20.9172 18.2097C20.9851 17.5634 20.5307 16.775 19.6212 15.1999L14.7715 6.79986C13.8621 5.22468 13.4071 4.43722 12.8135 4.17291C12.2957 3.94236 11.704 3.94236 11.1862 4.17291C10.5928 4.43711 10.1381 5.22458 9.22946 6.79845L4.37891 15.1999Z"
              stroke="currentColor"
              strokeWidth="1.5"
              strokeLinecap="round"
              strokeLinejoin="round"
            ></path>{" "}
          </g>{" "}
        </g>
      </svg>
    );
  }

  static Clock({ className }) {
    return (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="24"
        height="24"
        viewBox="0 0 24 24"
        fill="none"
        stroke="#FF6B35"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
        className={className}
      >
        <circle cx="12" cy="12" r="10"></circle>
        <polyline points="12 6 12 12 16 14"></polyline>
      </svg>
    );
  }

  static Location({ className }) {
    return (
      <svg
        xmlns="http://www.w3.org/2000/svg"
        width="18"
        height="18"
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
        strokeLinecap="round"
        strokeLinejoin="round"
        className={className}
      >
        <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"></path>
        <circle cx="12" cy="10" r="3"></circle>
      </svg>
    );
  }
}
