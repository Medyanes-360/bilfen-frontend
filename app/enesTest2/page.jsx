...
//               EÖ
//             </div>
//             <h2 className="text-white font-semibold ml-2 hidden sm:block opacity-90 group-hover:opacity-100 transition-opacity duration-300">
//               EğitimHub
//             </h2>
//           </div>
...
//               <h1 className="font-bold group-hover:text-blue-100 transition-colors duration-300">
//                 Merhaba, Ayşe Öğretmen
//               </h1>
//               <p className="text-xs text-blue-100 group-hover:text-white transition-colors duration-300">
//                 23 Mart 2025, Cuma
//               </p>
//             </div>
      ...
      if (todayElement) {
        setTimeout(() => {
          ...
          const scrollPosition =
            todayElement.offsetLeft - containerWidth / 2 + todayWidth / 2;
        ...
        {days.map((day, index) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-xl sm:text-2xl font-bold mb-1 transition-transform duration-300 group-hover:scale-110">
                {day.date}
              </span>
  ...
  return (
    <div className="group flex border border-gray-200 rounded-lg overflow-hidden bg-white hover:shadow-lg transition-all duration-300 ease-in-out hover:-translate-y-1 hover:border-blue-200">
      ...
      <div className="flex flex-1 flex-col sm:flex-row p-4 gap-3 sm:gap-0">
        {/* İkon Alanı */}
        <div
          className={`mr-0 sm:mr-4 ${typeInfo.bgColor} ${typeInfo.color} p-3 rounded-lg flex items-center justify-center border ${typeInfo.borderColor} group-hover:shadow-md transition-all duration-300 ease-in-out group-hover:scale-105 w-fit self-start`}
        >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-2">
            <div className="min-w-0 pr-1">
              <h3 className="font-semibold text-gray-800 text-sm sm:text-base flex items-center flex-wrap gap-1 truncate group-hover:text-blue-700 transition-colors duration-300">
                ...
                {/* {material.feedbackStatus && (
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full whitespace-nowrap
                      ${
                        material.feedbackStatus === "pending"
                          ? "bg-yellow-100 text-yellow-700 group-hover:bg-yellow-200"
                          : material.feedbackStatus === "evaluated"
                          ? "bg-blue-100 text-blue-700 group-hover:bg-blue-200"
                          : material.feedbackStatus === "approved"
                          ? "bg-green-100 text-green-700 group-hover:bg-green-200"
                          : material.feedbackStatus === "rejected"
                          ? "bg-red-100 text-red-700 group-hover:bg-red-200"
                          : ""
                      }
                    `}
                  >
                    {material.feedbackStatus === "pending" && "🕒 Beklemede"}
                    {material.feedbackStatus === "evaluated" && "🔍 Değerlendiriliyor"}
                    {material.feedbackStatus === "approved" && "✅ Onaylandı"}
                    {material.feedbackStatus === "rejected" && "❌ Reddedildi"}
                  </span>
                )} */}
              </h3>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mt-4 gap-3 sm:gap-0">
            ...
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-2 w-full sm:w-auto">
              ...
              {/* <button className="w-full sm:w-auto px-4 py-2 border border-blue-600 text-blue-600 rounded flex items-center justify-center text-xs sm:text-sm font-medium hover:bg-blue-50 hover:text-blue-700 active:bg-blue-100 active:scale-95 transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50">
                {feedbackIcon}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const todaysMaterials = MATERIALS_DATA.filter(
      (material) =>
        material.date ===
  ...
  const handleDateChange = (dateId) => {
    ...
    const selectedDateMaterials = MATERIALS_DATA.filter(
      (material) =>
        material.date ===
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <div className="flex flex-1 pt-0">
        ...
        {/* <aside
          className={`bg-white border-r border-gray-200 w-64 fixed lg:sticky inset-y-0 left-0 transform ${
            sidebarOpen ? "translate-x-0" : "-translate-x-full"
          } lg:translate-x-0 transition-transform duration-300 ease-in-out overflow-y-auto z-20 top-16 h-[calc(100%-4rem)] md:h-auto md:top-16`}
        >
      ...
      {/* <div
        className={`fixed inset-0 bg-black bg-opacity-15 backdrop-blur-sm z-10 transition-opacity duration-300 lg:hidden ${
